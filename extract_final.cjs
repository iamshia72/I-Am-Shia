...
// Extract Section One (Ziyarat After Salat)
const section1Match = html.match(/id="one"[\s\S]*?>([\s\S]*?)<\/div>(?:\s*<\/div>)?/i);
const section2Match = html.match(/id="two"[\s\S]*?>([\s\S]*?)<\/div>(?:\s*<\/div>)?/i);

if (section1Match) {
    const section1Html = section1Match[1];
...
if (section2Match) {
    const lines = extractSectionLines(section2Match[1]);
    fs.writeFileSync('lines_tasbih.json', JSON.stringify(lines, null, 2));
}
